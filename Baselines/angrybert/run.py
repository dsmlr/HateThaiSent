...
from nn import AngryBERT
from loss import setup_optimizer, MultiTaskLoss
from metrics import evaluate_model_sklearn_with_accuracy_and_reports


dataset_path = "dataset.csv"
fasttext_model_path = "path_to_fasttextmodel"
max_len_ft = 509
...
embedding_dict_from_id, embedding_dict_from_token= get_ft_embedding_dicts(tokenized_sent, fasttext_model_path)


token_to_id = {token: idx for idx, token in enumerate(embedding_dict_from_token.keys())}
id_to_token = {idx: token for token, idx in token_to_id.items()}


token_ids_sentences = [pad_and_tokens_to_ids(sentence, embedding_dict_from_id, token_to_id, actual_max_seq_len) for sentence in tokenized_sent]
embedding_matrix = np.array(list(embedding_dict_from_id.values()))
...
token_ids_sentences_train, token_ids_sentences_test, input_ids_train, input_ids_test, attention_masks_train, attention_masks_test, hate_label_train, hate_label_test, sent_label_train, sent_label_test= train_test_split(token_ids_sentences, bert_input_ids, bert_attn_masks , hate_labels, sent_labels, test_size = 0.20, random_state = 42)

train_ds = Multitask_Dataset(nn_tokens=token_ids_sentences_train, input_ids=input_ids_train, attention_masks=attention_masks_train, labels_task1=hate_label_train, labels_task2=sent_label_train)
train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)

test_ds = Multitask_Dataset(nn_tokens=token_ids_sentences_test, input_ids=input_ids_test, attention_masks=attention_masks_test, labels_task1=hate_label_test, labels_task2=sent_label_test)
test_loader = DataLoader(test_ds, batch_size=batch_size, shuffle=True)


model = AngryBERT(embedding_dict_from_id)

model.embedding_task1.init_embedding()
model.embedding_task2.init_embedding()

model = model.to(device)

criterion = MultiTaskLoss(device = device)

optimizer = setup_optimizer(model, criterion)
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        total_loss = criterion(logits_task1, logits_task2, batch_labels_task1.to(device), batch_labels_task2.to(device))
